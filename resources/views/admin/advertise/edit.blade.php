        ...
        <form class="form-horizontal" method="post" action enctype="multipart/form-data">
        	...
        	<div class="form-group {{ hasValidator('title') }}">
				...
				<div class="col-sm-6 text-center">
					<img style="max-width: 300px;" src="{{ PATH_IMAGE_ADVERTISE . $data->getBanner() }}">
				</div>
			</div>
            ...
            <div class="form-group">
				...
				<div class="col-sm-6 text-center">
					<select class="select2 form-control" name="user_id">
						...
						<option value="{{ $value->getId() }}" {{ (Request::old('user_id', $data->getUserId()) == $value->getId()) ? 'selected' : '' }}>{{ $value->getEmail() }}</option>
						@endforeach
					</select>
				</div>
			</div>
            ...
            <div class="form-group {{ hasValidator('title') }}">
				...
				<div class="col-sm-6 text-center">
					<input type="text" class="form-control" name="title" value="{{ Request::old('title', $data->getTitle()) }}">
					{!! alertError('title') !!}
				</div>
			</div>
            ...
            <div class="form-group {{ hasValidator('link') }}">
				...
				<div class="col-sm-6 text-center">
					<input type="text" class="form-control" name="link" value="{{ Request::old('link', $data->getLink()) }}">
					{!! alertError('link') !!}
				</div>
			</div>
				...
				<div class="col-sm-6 text-center">
					<select type="text" class="form-control" name="position">
						@foreach(position_ads() as $key => $value)
							<option value="{{ $key }}" {{ Request::old('position', $data->getPosition()) == $key ? 'selected' : '' }}>{{ $value }}</option>
						@endforeach
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-3 text-center">
					<input type="text" class="form-control datepicker" value="{{ Request::old('created_at', date('d/m/Y', strtotime( $data->getCreatedAt() ))) }}" name="created_at">
					{!! alertError('created_at') !!}
				</div>
				<div class="col-sm-3 text-center">
					<input type="text" class="form-control datepicker" value="{{ Request::old('expired', date('d/m/Y', strtotime( $data->getExpired() ))) }}" name="expired">
					{!! alertError('expired') !!}
				</div>
			</div>
